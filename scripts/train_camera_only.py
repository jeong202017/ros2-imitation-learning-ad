#!/usr/bin/env python3
"""
Camera-Only Model Training Script.

Trains CNN model using only camera images for control prediction.
카메라 이미지만 사용하여 제어 예측을 위한 CNN 모델을 학습합니다.
"""
...
from data_processing.dataset_camera import CameraOnlyDataset
...
class CameraOnlyTrainer:
    """Trainer for camera-only model.
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Train camera-only model",
...
def main():
    ...
    train_dataset = CameraOnlyDataset(
        data_dir=args.data_dir,
        split="train",
        normalize=True,
    )
    
    val_dataset = CameraOnlyDataset(
        data_dir=args.data_dir,
        split="val",
        normalize=True,
    )
    ...
    model = create_cnn_policy(config["model"])
    logger.info(f"Model created with {sum(p.numel() for p in model.parameters())} parameters")
    
    # Create trainer
    trainer = CameraOnlyTrainer(
        model=model,
